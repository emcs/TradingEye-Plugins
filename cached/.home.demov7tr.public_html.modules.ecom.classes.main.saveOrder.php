...
#CONSTRUCTOR
		...
		if($this->payTotal>0) {
			switch($this->payMethod) {		
				...
				break;
								...
								case "paypal_express":
					$paypalExpress = new c_Paypal_Express();
					$paypalExpress->obDb = $this->obDb;
					$paypalExpress->obTpl = $this->obTpl;
					$paypalExpress->libFunc = $this->libFunc;
					$paypalExpress->DoExpressCheckout($this->orderId);
					exit;
				break;
				case "secpay":
					$this->secpaySubmit();
	...
	function barclaySubmit(){		
		$this->ObTpl=new template();
		$this->ObTpl->set_file("TPL_BARCLAY_FILE",$this->barclayTemplate);
		$this->ObTpl->set_var("TPL_VAR_SITEURL",SITE_SAFEURL);
		$this->ObTpl->set_var("TPL_VAR_SESSIONID",session_id());
		$this->ObTpl->set_var("TPL_VAR_ITEMNUMBER",$this->orderId);
		...
		$CpiDirectResultUrl = SITE_SAFEURL."barclay/barclay.php&oid=".$this->orderId;
		$this->ObTpl->set_var("CpiDirectResultUrl",$CpiDirectResultUrl);
		...
		//$params="clientid=25991";//.$_CONF['CPI_ID'];
		$params="clientid=".BARCLAYS_CLIENTID;   
		//$params.="&password=0987654321123456";//.$_CONF['CPI_PASSWORD'];
		$params.="&password=".BARCLAYS_PASSWORD;
		//	$params.="&oid=45";//.$tmpNextVal;
		$params.="&oid=".$this->orderId;
		$params.="&chargetype=Auth";
		$params.="&currencycode=".PAYMENT_CURRENCY;
		//.$_CONF['CPI_CURRENCYCODE'];
		//$params.="&total=0.01";	
		$params.="&total=".$this->payTotal;	
		$strEPDQ="yes";

		#perform the HTTP Post
		$response = $this->m_PullPage($server,$url,$params);
		...
		//submit the encrypted string
		$this->ObTpl->set_var("TPLVAR_ENCRYPTEDCODE",$strEPDQ);
	
		
		$timestamp=time();
		$timestamp*=1000;
		$this->ObTpl->set_var("TPL_VAR_TIMESTAMP",$timestamp);
		$this->ObTpl->set_var("TPL_VAR_MERCHANT_DATA",session_id());
		
		#SETTING BILL STATENAME
		if($this->libFunc->ifSet($_SESSION,'bill_state_id','0'))
		{
			$this->obDb->query = "SELECT vShortName,vStateName FROM ".STATES." where iStateId_PK  = '".$_SESSION['bill_state_id']."'";
			$row_state = $this->obDb->fetchQuery();
			if($_SESSION['bill_country_id']==220){
				$this->ObTpl->set_var("TPL_VAR_BILLSTATE",$this->libFunc->m_displayContent($row_state[0]->vShortName));
				$this->ObTpl->set_var("TPL_BILLSTATE_ARGU",'bstate');
			}
			else{
				$this->ObTpl->set_var("TPL_VAR_BILLSTATE",$this->libFunc->m_displayContent($row_state[0]->vStateName));
				$this->ObTpl->set_var("TPL_BILLSTATE_ARGU",'bcountyprovince');
			}
		}
		else
		{
		...
		$row_country = $this->obDb->fetchQuery();
		$this->ObTpl->set_var("TPL_VAR_BILLCOUNTRY",
		$this->libFunc->m_displayContent($row_country[0]->vShortName));
		
		#SETTING SHIP STATENAME
		if($this->libFunc->ifSet($_SESSION,'ship_state_id','0'))
		{
			$this->obDb->query = "SELECT vShortName,vStateName FROM ".STATES." where iStateId_PK  = '".$_SESSION['ship_state_id']."'";
			$row_state = $this->obDb->fetchQuery();
			if($_SESSION['bill_country_id']==220){
			$this->ObTpl->set_var("TPL_VAR_SHIPSTATE",$this->libFunc->m_displayContent($row_state[0]->vShortName));
			$this->ObTpl->set_var("TPL_SHIPSTATE_ARGU",'sstate');
			}
			else{
				$this->ObTpl->set_var("TPL_VAR_SHIPSTATE",$this->libFunc->m_displayContent($row_state[0]->vStateName));
				$this->ObTpl->set_var("TPL_SHIPSTATE_ARGU",'scountyprovince');
		...
		$this->ObTpl->pparse("return","TPL_BARCLAY_FILE");
	}#EF
	
	function hsbcSubmit() {
	   	//this equates to 28.00 if u wanted 15.50 it would be 1550, easiest way to get the number is * the number by 100, that will get rid of the decimal.
	    $this->payTotal=$this->payTotal*100;
		$PurchaseAmount = $this->payTotal; 
		...
		//$StorefrontId = "UK06809341GBP"; //ur unique storefrontid, supplied by HSBC
		$StorefrontId = HSBC_STOREID;
		//$hashKey = "prO7bhW0m/j5McbJRSixZUvgSRJKHY+Z"; //32 digit hash key supplied by HSBC
		$hashKey =HSBC_KEY;
		//$CpiDirectResultUrl = SITE_SAFEURL."ecom/index.php?action=checkout.process&mode=".$this->orderId."&phpsessid=".session_id();
		//$CpiReturnUrl = SITE_SAFEURL."ecom/index.php?action=checkout.process&mode=".$this->orderId."&phpsessid=".session_id();
		
		$CpiReturnUrl = SITE_SAFEURL."ecom/index.php?action=checkout.review&mode1=cancel&errhsbc=1";
		$CpiDirectResultUrl = SITE_SAFEURL."ecom/index.php?action=checkout.review&mode1=cancel&errhsbc=1";	
		
		$PurchaseCurrency = HSBC_CURRENCY; //826 is the number for GBP
		$Type = "Capture"; //Optional, either Auth (have to flag as authorised on the virtual terminal) or Capture (Automatically marks the funds for settlement)
		$Mode = HSBC_MODE; // T for Test, P for Production
		$TimeStamp=time(); 
		//$TimeStamp*= 1000; //Time has to be in microseconds, hence the *1000
		$TimeStamp.='000';

		$BillingAddress1=$this->libFunc->m_displayContent($_SESSION['address1']);
		
		$BillingCity=$this->libFunc->m_displayContent($_SESSION['city']);
		$BillingCountry="826";
//		$BillingCountry=$this->m_countryName($_SESSION['bill_country_id'],1);
		
		$BillingFirstName=$this->libFunc->m_displayContent($_SESSION['first_name']);
		$BillingLastName=$this->libFunc->m_displayContent($_SESSION['last_name']);
	    $BillingPostal=$this->libFunc->m_displayContent($_SESSION['zip']);
		$ShopperEmail=$this->libFunc->m_displayContent($_SESSION['email']);
		$ShippingAddress1=$this->libFunc->m_displayContent($_SESSION['alt_address1']);
		$ShippingCity=$this->libFunc->m_displayContent($_SESSION['alt_city']);
		$ShippingCountry="826";
		//$ShippingCountry=$this->m_countryName($_SESSION['ship_country_id'],1);
		$ShippingLastName=$BillingLastName;
		$ShippingFirstName=$this->libFunc->m_displayContent($_SESSION['alt_name']);
		$ShippingPostal=$this->libFunc->m_displayContent($_SESSION['alt_zip']);
		
		
		$teststr = "\"$CpiDirectResultUrl\" \"$CpiReturnUrl\" \"$Type\" \"$OrderDesc\" \"$OrderId\" \"$PurchaseAmount\" \"$PurchaseCurrency\" \"$StorefrontId\" \"$UserId\" \"$Mode\" \"$TimeStamp\" \"$BillingAddress1\" \"$BillingCity\" \"$BillingCountry\" \"$BillingFirstName\" \"$BillingLastName\" \"$BillingPostal\" \"$ShopperEmail\" \"$ShippingAddress1\" \"$ShippingCity\" \"$ShippingCountry\" \"$ShippingLastName\" \"$ShippingFirstName\" \"$ShippingPostal\""; 
		...
		//edited by anjali
		echo "<form name=\"hsbc_form\" method=\"post\" action=\"https://www.cpi.hsbc.com/servlet\">
		<input type=\"hidden\" name=\"StorefrontId\" value=\"$StorefrontId\">
		<input type=\"hidden\" name=\"OrderHash\" value=\"$generated_Hash\">
		<input type=\"hidden\" name=\"TransactionType\" value=\"Capture\">
		<input type=\"hidden\" name=\"CpiDirectResultUrl\" value=\"$CpiDirectResultUrl\">
		...
		<input type=\"hidden\" name=\"BillingPostal\" value=\"$BillingPostal\">
		...
		<input type=\"hidden\" name=\"ShippingPostal\" value=\"$ShippingPostal\">
		<input type=\"hidden\" name=\"ShippingFirstName\" value=\"$ShippingFirstName\">
		<input type=\"hidden\" name=\"Mode\" value=\"$Mode\">
		<input type=\"submit\" value=\"Click here if you do not get automatically redirected in 10 seconds...\"></form><script>document.hsbc_form.submit();</script>";
		die;
	}
	
	function m_PullPage( $host, $usepath, $postdata = "" )
		{	 
